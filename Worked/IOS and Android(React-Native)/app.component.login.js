...
var LoginComponent = React.createClass({
    ...
    perform_login: function() {
        var context = this;
        server_auth.authenticate_username_password("https://dev.tractfile.com/", this.state.username, this.state.password).then(() => {
	...
	perform_submit: function() {// please your fill infora submit click
        ...
        if (this.state.username1.indexOf("@") == -1) {
		 ...
		 fetch("https://dev.tractfile.com/api/auth/reset_password", {
                method: "POST",				
                headers: {   
	...
	on_submit_blue: function(){ //text change event
		...
		var context = this;
         fetch("https://dev.tractfile.com/api/auth/get_secret_question", {
                method: "POST",				
                headers: {           
                "Accept":"application/json",
                "Content-Type":"application/json"
    ...
    render: function() {
       if(this.state.forgetview=="no"){
        ...
        return (
            <View style={{ flex: 1, justifyContent: 'center',  alignItems: 'center',   backgroundColor:'#B3C6FF',
               }} >
				<View style={styles.top_container}>
				    <Image
					source={{uri: "https://dev.tractfile.com/images/logo.png"}}					
					style={{ width:180, height: 45,padding:5,top:30, }}
				  />
				</View>
		    ...
		    return(
			   <View style={{ flex: 1, justifyContent: 'center',  alignItems: 'center', backgroundColor:'#B3C6FF',}} >
				   <View style={styles.top_container}>
						<Image
						source={{uri: "https://dev.tractfile.com/images/logo.png"}}					
						style={{ width:180, height: 45,padding:5,alignSelf:'stretch',top:30, }}
					  />
					</View>
					 <View style={styles.container}>
						 <TouchableOpacity  onPress={this.error_clear}>
