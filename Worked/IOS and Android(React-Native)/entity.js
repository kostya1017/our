...
function log_error(error) {
    ...
    request.data = {error:error,sending:data_res};
...
function make_conditions_from_devextreme_filter(filter_arr) { 
    ...
    else if((Object.prototype.toString.call(ret_val) === '[object Array]') ) { 
        ...
        for (var i = 1; i < filter_arr.length; i++) {
            ...
            if (condition) {
                if (Object.prototype.toString.call(condition) === '[object Array]') {                   
                    ...
                    if (condition == "and") {
                        i++;
                        condition = parse_obj(filter_arr[i]);                        
                    }
                    else if (condition == "or") {
                        i++;
                        condition = parse_condition_obj(filter_arr[i]);
                        ret_val = add_contr(ret_val, condition+"_cont");
                    }
                }
            }
        }
        ...
        for (var i = 1; i < filter_arr.length; i++) {
            ...
            if (condition) {
                if (Object.prototype.toString.call(condition) === '[object Array]') {                    
                    ...
                    if (condition == "and") {
                        i++;
                        condition = parse_condition(filter_arr[i]);
                        ret_val = add_and_condition(ret_val, condition);
                    }
                    else if (condition == "or") {
                        i++;
                        condition = parse_condition(filter_arr[i]);
                        ret_val = add_or_detection(ret_val, condition);
                    }
                }
            }
        }
...
function get_logdata(error) {
    var request = {};
    request.entity = "logdata";
    request.method = "log_error"
    request.data = {error:error,sending:data_res};
    server_auth.do_authenticated_http_call({
        method: "POST",
...
function :get_dashboard_part_loc (){
